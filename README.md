...
- **能展示**：静态站内有开屏视觉、地形图、AI 架构图、战役回放和真实发布结果。
...
| 提交 ZIP | 可上传打包产物 | [Release ZIP](https://github.com/LongWeihan/clawforge-community-pioneer/releases/download/v1.0.0/clawforge-community-pioneer-submission.zip) |

## 当前演示案例

本次提交选择了 **Agent Cron Token Saver 开荒战役** 作为 MVP 案例。
...
ClawForge 的 MVP 没有为了比赛强行起复杂后端，而是采用“角色化 Agent 函数 + 静态数据契约 + 可视化回放”的实现方式。这样更稳定，能直接挂到 GitHub Pages，也便于评委打开即看。
...
| 截图素材 | README、提交页和展示页使用的演示截图 | `screenshots/` |
...
- **Campaign Replay**：让评委能快速回看每个 Agent 做了什么。
...
- `scripts/check-secrets.mjs` 会扫描明显 API Key，避免密钥进入提交包。
...
这个 MVP 已经能作为黑客松展示作品提交。后续如果继续做，可以沿三条线增强：
...
> ClawForge 不是又做了一个技能，而是给 EasyClaw 做了一台早期社区开荒机器：它能发现缺口、生产 Skill、证明有用、发帖传播、播种悬赏，并把反馈变成下一轮增长。
